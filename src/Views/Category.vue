  ...
  >
    <div>
      <div class="flex gap-3">
        <h1 title="Category Name" class="text-xl font-semibold text-light-0">
          {{ Category.nom }}
        </h1>
      ...
      <p title="Description" class="text-light-2">{{ Category.description }}</p>
      ...
      <div v-if="Category.date_modification" class="flex gap-5">
        <h2 class="font-semibold text-base text-light-1">Date Created :</h2>
...
async function fetch() {
  await axios
    .get(`http://192.168.1.12:8080/api/categories/${route.params.id}`)
